...
## 6.5 LLM 사용량 계측 — 서버·서비스 리소스 산정

`src/llm_meter.py`가 HCX 호출마다 한 줄씩 기록한다. 기록 단위는 **호출 1회**다(문장 1개가 아니다) — 한 문장이 수리·재샘플로 최대 3콜을 쓰므로, 재시도가 비용에서 차지하는 몫이 보여야 튜닝의 손익을 판단할 수 있다.

```bash
venv\Scripts\python.exe -m src.p3_stage_b --spike 20 --fresh   # 실호출로 실측(과금)
venv\Scripts\python.exe -m src.llm_meter --report --articles 2695 --concurrency 4
venv\Scripts\python.exe -m src.llm_meter --breakdown 15        # 입력 토큰 구성(과금 없음)
```

- 계측은 **기본 켜짐**이고 `--no-meter`로 끈다. 기록 실패가 파이프라인을 죽이지 않는다(부수효과).
- `--fresh`는 캐시 재생을 꺼서 전부 실호출한다 — 캐시가 있으면 토큰·지연을 잴 수 없기 때문이다. **동결 캐시가 오염되지 않도록 `--cache`로 별도 경로를 주고 쓸 것.**
- 캐시 재생도 한 줄 남긴다(요금 0). "실호출 대비 얼마나 아꼈나"가 보인다.
- 토큰은 **추정하지 않는다** — 응답의 `usage.promptTokens`·`usage.completionTokens`를 그대로 쓴다(과금 기준값과 동일). 없으면 `null`로 두고 집계에서 뺀다. 0으로 채우면 요금이 조용히 과소 집계된다.

**단가** (`API/CLOVA_요금.pdf`, 1,000토큰당 KRW, VAT 별도)

| 모델 | 입력 | 출력 |
|---|---|---|
| **HCX-005**(현행) · HCX-007 | 1.25원 | 5원 |
| HCX-DASH-002 | 0.25원 | 1원 |
| HCX-003 | 5원(입출력 통합) | |

**실측 (2026-08-13 · dev 8기사 104문장 전량 실호출 · `extract_v1.8`)**

| 항목 | 값 | | 항목 | 값 |
|---|---|---|---|---|
| API 호출 | 129회 (문장당 1.24) | | 평균 지연 | 2.29초 |
| 입력 토큰 | 484,672 (호출당 3,757) | | 중앙값 | 1.95초 |
| 출력 토큰 | 10,470 (호출당 81) | | p95 | 4.31초 |
| 요금 | 658.19원 (VAT 별도) | | 출력 처리량 | 35.4 tok/s |
| 기사 1건당 | **82.27원 · 16.1콜 · 37초** | | 문장 1개당 | 6.33원 |

> **⚠ 비용을 지배하는 것은 입력이다.** 단가는 출력이 4배 비싸지만 물량이 입력:출력 = **46:1**이라 요금의 **92%가 입력**에서 나온다. 그리고 그 입력의 **92.6%가 시스템 프롬프트**(3,366토큰)로, 매 호출 통째로 반복된다. 즉 **전체 요금의 약 85%가 반복 전송되는 프롬프트**다.
> 비용을 줄이려면 출력 길이가 아니라 **시스템 프롬프트(few-shot 6건)를 먼저 본다.** 30% 줄이면 1,000콜당 약 1,262원이 준다. 단, few-shot을 걷어내면 추출 품질이 떨어질 수 있으므로 축약판은 **dev 채점으로 손실을 확인한 뒤** 채택한다(§5.6의 DASH 절감 경로와 같은 규율).

**시도 유형별 비용 분해** — 재시도가 총 요금의 **20%**를 쓴다.

| 유형 | 호출 | 요금 | 비중 |
|---|---|---|---|
| `initial` | 104 | 526.17원 | 79.9% |
| `stage_c_repair` | 17 | 90.29원 | 13.7% |
| `repair` | 6 | 31.66원 | 4.8% |
| `resample` | 2 | 10.07원 | 1.5% |

Stage C 검증 실패로 인한 재추출이 재시도 비용의 대부분이다 — **추출 품질을 올리면 요금이 같이 내려간다.**

**규모 추정** (기사당 실측치를 선형 확장 — 기사 길이 분포가 표본과 다르면 그만큼 어긋난다)

| 대상 | 호출 | 요금(VAT 별도) | 소요(동시성 4) |
|---|---|---|---|
| 선별 52건 | 약 840회 | 약 4,280원 | 약 8분 |
| news.csv 전량 2,695건 | 약 43,500회 | **약 221,700원** | 약 6.9시간 |

> 동시성을 올리면 시간은 비례해 줄지만 **요금은 그대로**다. 반대로 프롬프트를 줄이면 둘 다 준다.
...
│   ├── llm_meter.py          (310줄) 사용량 계측 — 토큰·요금·지연 기록과 집계
...
| 요금 단가 갱신 | `llm_meter.py`의 `PRICING_PER_1K` (테스트가 요금표와 1:1로 고정) |
...
```bash
venv\Scripts\python.exe -m src.p3_stage_b --spike 20 --fresh --cache cache\meter_run.jsonl
```

`--fresh` 없이는 캐시가 재생돼 실측이 되지 않는다. 동결 캐시를 지키려면 `--cache`로 별도 경로를 준다.
